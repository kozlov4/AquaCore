...
class Tasks(Base, TableNameMixin):
    id: Mapped[int_pk]
    user_id: Mapped[int] = mapped_column(BIGINT, ForeignKey('users.id'), nullable=False)
    aquarium_id: Mapped[Optional[int]] = mapped_column(BIGINT, ForeignKey('aquariums.id'))
    ...
    aquarium: Mapped[Optional["Aquariums"]] = relationship(back_populates="tasks")
    completions: Mapped[list["Task_Completions"]] = relationship(back_populates="task")

class Task_Completions(Base, TableNameMixin):
    id: Mapped[int_pk]
    task_id: Mapped[int] = mapped_column(BIGINT, ForeignKey('tasks.id'), nullable=False)
    completion_date: Mapped[timestamp_now] = mapped_column(nullable=False)

    task: Mapped["Tasks"] = relationship(back_populates="completions")