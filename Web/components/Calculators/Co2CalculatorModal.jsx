...
export function Co2CalculatorModal({ onClose }) {
  ...
  return (
    <CalculatorModalLayout
      ...
      onClose={onClose}
    >
      <div className="grid grid-cols-[1fr_190px] gap-7 px-6 py-6">
        <div className="space-y-5">
          <CalculatorInput label="Рівень кислотності (pH)" value={ph} onChange={setPh} unit="" />
          <CalculatorInput label="Карбонатна жорсткість (kH)" value={kh} onChange={setKh} unit="°dKH" />
          <p className="text-xs text-slate-400">Формула: 3 × kH × 10^(7 - pH)</p>
        </div>
