...
export function CalculatorCard({ calculator, index, onOpen }) {
  ...
  return (
    <motion.article
      ...
      className="group flex min-h-[215px] flex-col justify-between rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition"
    >
      <div>
        <div
          className={`mb-6 flex h-14 w-14 items-center justify-center rounded-2xl text-3xl ${accent.iconBg}`}
        >
          {calculator.icon}
        </div>
      ...
      <motion.button
        ...
        whileHover={{ x: 4 }}
        whileTap={{ scale: 0.96 }}
        className={`mt-6 flex items-center gap-2 text-sm font-black ${accent.link}`}
      >
        Відкрити
        <ArrowRight size={16} />
      </motion.button>
