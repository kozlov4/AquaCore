...
function getAquariumName(aquarium) {
  return aquarium?.name || "Акваріум";
}

function getAquariumType(aquarium) {
  return aquarium?.type || aquarium?.environment || aquarium?.raw?.type || "Прісноводний";
}
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-5 top-5 rounded-full bg-white/20 px-4 py-2 text-sm font-medium text-white backdrop-blur-md">
        ...
        {isLoading ? "Завантаження..." : `Стан: ${status}`}
      </div>

      <div className="absolute bottom-6 left-6 text-white">
        <h1 className="text-3xl font-bold">{isLoading ? "Завантаження..." : name}</h1>

        <p className="mt-2 text-sm text-white/80">
          {type} • {volume} • Запущений: {createdDate}
        </p>
      </div>
    </motion.section>
  );
